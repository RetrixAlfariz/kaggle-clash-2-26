...
from sequence_crf import tokens, gold_tags, spans_from_tags, model_arrays, emissions, decode_k
from sequence_crf_dictionary import features
...
def main():
    ...
    p.add_argument('--train-docs',type=int,default=24000)
    p.add_argument('--iterations',type=int,default=80)
    p.add_argument('--output',type=Path,default=ROOT/'output/bitrase-1.algo-4/crf24k')
    args=p.parse_args(); out=args.output.resolve()
    if out.exists(): raise ValueError('Preserve previous run')
    out.mkdir(parents=True)
    paths=['src/sequence_crf.py','src/run_sequence_crf_dictionary.py','src/sequence_crf_dictionary.py','src/run_m0.py','src/submit_m0.py','src/baseline_probe.py','output/m0/v1/dictionary.parquet','src/prepared_loader.py','src/ner_evaluation.py',
           'output/prepared/v1/train.parquet','output/prepared/v1/dev.parquet','note/bitrase-1.algo-4/protocol.md']
    ...
    config={'experiment':'bitrase-1.algo-4','method':'linear-chain BIO CRF from scratch',
