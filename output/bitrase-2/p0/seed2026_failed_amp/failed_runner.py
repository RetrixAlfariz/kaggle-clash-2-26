...
def main():
    ...
    scaler=torch.amp.GradScaler('cuda');logs=[];results={};selected_documents={}
    for epoch in range(1,a.epochs+1):
        head.train();order=list(range(len(plans)));random.Random(a.seed+epoch).shuffle(order)
        total=0.;population=0;strata={name:[0.,0,0] for name in ('positive','near','old_fp','random')};epstart=time.monotonic()
        for begin in range(0,len(order),32):
            ...
            scaler.step(opt);scaler.update();population+=denom
            if begin%(32*100)==0:print(f'Epoch{epoch} train {min(begin+32,len(order))}/{len(order)} weighted_ce={total/population:.5f}',flush=True)
        log={'epoch':epoch,'weighted_ce':total/population,'seconds':time.monotonic()-epstart,
